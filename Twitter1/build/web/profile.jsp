        ...
        <div class="wrapper">
            ...
            <div class="middle">
                <div id="container">
                    <main class="content">
                        <div id="profile">
                            <h2>Name: <% out.println(((User) session.getAttribute("user")).getName());%><br><br></h2>
                            <h2>Surname: <% out.println(((User) session.getAttribute("user")).getSurname());%></h2>
                            <h2>Login: <% out.println(((User) session.getAttribute("user")).getLogin());%></h2>
                            <h2>Mail address: <% out.println(((User) session.getAttribute("user")).getEmail());%>
                        </div>
                    </main><!-- .content -->
                </div><!-- .container-->
                ...
                <aside id="right-sidebar">
                    <%  
                        List<User> list = db.getFollowers(((User) session.getAttribute("user")).getId());
                    %>
                    <h2>You follow: <br></h2>
                    <%
                        for (User u : list) {
                            out.println("<a href = 'follower.jsp?id=" + u.getId() + "'>" + u.getName() + " " + u.getSurname() + "</a>");
                            out.println("<br>");
                        }
                    %>
                    <h2>    You are followed by: <br></h2>
                    <%
                        List<User> list1 = db.getFollows(((User) session.getAttribute("user")).getId());
                            for (User user : list1) {
